...
export class OcelinAssistant extends ClawdAssistant {
  connectedCallback() {
    if (!this._ocelin) {
      ...
      style.textContent = `.ocelin-ear,.ocelin-tail{position:absolute;pointer-events:none}.ocelin-ear{z-index:1;transform-origin:center bottom}.ocelin-tail{z-index:0}.ocelin-ear.right{scale:-1 1}:host([compact]) .assistant-dock{display:none}:host([compact]) .assistant.clawd-assistant{--travel-start:-22px;--travel-end:-22px;left:-22px;bottom:-26px;transform:scale(.75);transform-origin:50% 100%}:host([compact]) .clawd-root{overflow:visible}`;
