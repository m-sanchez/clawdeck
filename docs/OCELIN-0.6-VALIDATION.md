...
- The 0.6.2 adapter replaces the generic taskbar glyph with the approved Ocelin pixel mascot and a compact status/RAM layout. State animations are rendered from the existing component; reduced-motion preferences select static assets. Visual verification of this revision remains separate from the working 0.6.1 text strip.
