...
- The 0.6.2 adapter replaces the generic taskbar glyph with the approved Ocelin pixel mascot and a compact status/RAM layout. State animations are rendered from the existing component; reduced-motion preferences select static assets. The refreshed package awaits user review, and its pet has not yet been visually confirmed in Explorer. The user-confirmed 0.6.1 text strip remains separate evidence.
...
- The packaged regression renamed the original launch directory while Ocelin and its helpers remained running. Smoke tests skip protocol registration; before/after registry checks confirmed the installed `ocelin://` association was unchanged. The package self-test and all 693 core tests passed after the fix.
