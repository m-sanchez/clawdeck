...
- The English Taskbar Widgets host and Ocelin widget 0.6.1 were installed. The user's approval was persisted. Its runtime then attached to both taskbars, but Ocelin remained disabled because upstream's stale catalog selected a built-in widget after installation. The setup patch starts the runtime and waits for the exact widget before enabling it; 11 mocked regressions cover delayed catalogs, updates and failures without changing permission approval. Inside-taskbar text still needs visual confirmation after enabling the widget.
- Packaged 0.6.1 desktop smoke passed. The anchored tile is movable, dragging releases its anchor, and its visible hide button and window close both persist the disabled preference. Restoring through preferences works. The physical drag gesture has not been verified in this run.
