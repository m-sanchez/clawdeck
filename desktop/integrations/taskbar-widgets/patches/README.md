...
The native primary-action patch lets a native widget without an expanded layout declare a root `layout.action`. A click sends that provider action with the actual widget instance ID; action names use the process provider's existing validation rules. The clickable root shows a hand cursor on hover and restores the previous cursor for dragging, exit and unload. Nested buttons, right-click Settings and expanded widgets retain their behavior. The user confirmed the pet is visible and adapter 0.6.4's whole-widget click opens the session panel with a visible slide. The cursor host passed CI run 35219044416 and is installed; its hand cursor still needs a live hover check.
